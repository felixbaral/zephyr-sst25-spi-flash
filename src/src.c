...
#define spi_nor_cmd_write_data(dev, opcode, src, length) \
	spi_nor_access(dev, opcode, NOR_ACCESS_WRITE, \
		       0, (void *)src, length)
			 ...
			 size_t size)
{
	...
	// 		ret = spi_nor_cmd_addr_write(dev, 0xAD,//SPI_NOR_CMD_PP, 
	// 		addr, src, to_write);
...
//######
	...
	{
		if (!written) {
			//spi_nor_access(dev,0xAD, NOR_ACCESS_WRITE,0, data_cmd, 4);
			spi_nor_cmd_addr_write(dev, 0xAD, addr, src, 2);
		}
			...
			// if (written <= 2)
			// 	// spi_nor_cmd_addr_write(dev, 0xAD, data, 2);
			// 	spi_nor_cmd_write_data(dev, 0xAD, src + written, 1);
			// else
				// spi_nor_cmd_addr_write(dev, 0xAD, data, 3);
				spi_nor_cmd_write_data(dev, 0xAD, src+ written, 2);
